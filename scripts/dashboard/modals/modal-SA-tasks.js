...
export default function openSATasks(tasksArray) {
  ...
  const remainingTasks = tasksArray.filter((task) =>
    isTodoPassedDeadline("task", task.time_box.to)
  );
  ...
  tasksArray
    ...
    .forEach(({ id, time_box, title, description, priority, done }) => {
      ...
      const isDoneValue = document.createTextNode(`${done ? "Yes" : "No"}`);
